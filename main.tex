...
\begin{thm}[Riesz Representation]
    % Linear functionals on a Hilbert space $\mathscr{H}$ are in one-to-one correspondence with the elements of $\mathscr{H}$ through $\mathscr{H} \to \mathscr{H}^*: x \mapsto \langle x, - \rangle$, where $\mathscr{H}^*$ denotes the dual space of $\mathscr{H}$, and $\langle -, - \rangle$ is the inner product on $\mathscr{H}$.
...
% , and $C^\mathbb{R}(X)$ for continuous functions $X \to \mathbb{R}$.

% The Borel sigma-algebra on a space $X$ will be denoted $\mathcal{B}(X)$.
...
% linear functionals can be uniquely represented by certain measures. In particular, 
...
\begin{proof}[Proof \cite{Reed_Simon_1980}, \cite{ruy_answer_2020}]
    ...
    % $$\left\|(\phi(f) - \lambda)\left(\frac{1}{\|\phi(g)\xi\|}\phi(g)\xi\right)\right\| \leq \|\phi(f) - \lambda\| < \frac{\epsilon}{2}$$
    % and
...
% More concretely, later on we will be interested in linear functionals of the form $f \mapsto \langle f(A) \psi, \psi \rangle$, and the unitary $U$ we construct in the following lemma will provide us with a way to pass from $f(A)\psi$ back to $f$.
...
% , so by the special case of the Riesz-Markov-Kakutani Representation Theorem for positive bounded linear functionals, for all $f \in C(\sigma(A))$ we have $\int _{\sigma(A)} f(\lambda) \, d\mu_{\psi}(\lambda) \in \mathbb{R}$.
% whenever $f \in C^{\mathbb{R}}(\sigma(A))$.
...
\end{thm}
\begin{proof}[Proof \cite{Reed_Simon_1980}]% TODO check
...
\begin{lem}\label{A-symm}
    $A$ is a symmetric operator. %TODO: Show $A$ is closed!
\end{lem}
\begin{proof}
